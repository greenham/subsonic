...
(function() {
  var Subsonic, request, fs, m3u8stream, ssdl;
  ...
  Subsonic = (function() {
    ...
    Subsonic.prototype.streamToFile = function(id, outputFile) {
      let out = fs.createWriteStream(outputFile);
      request.get("" + this.server + "/" + "stream").query({
        'u': this.username,
        'p': this.password
      }).query({
        'c': this.application,
    ...
    Subsonic.prototype.stream = function(link) {
      return ssdl(link);
    };
